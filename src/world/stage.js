...
const getGroundDimensions = (dataArray) => {
   ...
   return [groundHeight, groundWith]
}
...
export default function getStage(scene, dataArray, graph) {
    ...
    planes.forEach((mesh)=>{scene.add(mesh)})
    ...
    console.log(planes[0].position, planes[1].position, planes[1].rotation)

    planes[0].add(graph)
    graph.position.set( -groundWidth/3 , -groundHeight/3, 10)    
...
// const setStage = (sidePlanes, endPlanes) => {
//     const
